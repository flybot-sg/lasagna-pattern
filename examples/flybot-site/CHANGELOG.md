...
- A malformed lookup key, or a post field the schema does not declare, is a 403 `:schema-violation` instead of a silent `nil` or a 500 - [#38](https://github.com/flybot-sg/lasagna-pattern/issues/38).
...
- Mutation input validation via `coll/validated`: member and admin post writes and owner role grants are checked against closed write schemas

## [0.4.2] - 2026-06-29

### Fixed

- Style markdown tables in post content — tables in `.post-body`/`.post-content`/`.page-content` now render with borders, cell padding, a shaded header row, and responsive horizontal scroll instead of unstyled browser defaults

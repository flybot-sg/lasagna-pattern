...
- `:ex->error` option on `make-handler`/`wrap-api`/`execute`, hook receiving the original throwable and returning the wire error
- `default-ex->error`, the default exception → `:execution-error` conversion, composable from a custom hook

### Changed

- Remove `execute-mutation`'s redundant try/catch — `execute`'s catch is the single exception → wire-error conversion point (behavior unchanged)

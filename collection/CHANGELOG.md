...
- Runs on [ClojureCLR](https://github.com/clojure/clojure-clr); the wrappers implement `ILookup`, `Seqable` and `Counted` there too, and `bb clr-test collection` runs the RCT suite. `validated` and `sample` stay JVM/JS only (Malli) - [#34](https://github.com/flybot-sg/lasagna-pattern/issues/34).
